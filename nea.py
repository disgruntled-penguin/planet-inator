...
class NEALoader:
    """Handles loading and filtering Near Earth Asteroids from JSON data"""
    ...
    def load_asteroids(self, limit: Optional[int] = None):
        """Load asteroids from JSON file with optional limit"""
    ...
    def print_sample_data(self, num_samples: int = 3):
        """Print sample asteroid data to check if loading worked correctly"""
        if not self.loaded:
    ...
    def get_interesting_asteroids(self, count: int = 50):
        """Get a selection of interesting/famous asteroids"""
        if not self.loaded:
            self.load_asteroids(limit=10000)  # Load a subset first
        
        # Look for famous/interesting asteroids
        interesting_names = [
            'Eros', 'Albert', 'Apophis', 'Bennu', 'Itokawa', 'Ryugu',
        ...
        for asteroid in self.asteroids:
            ...
            if any(interesting_name in name for interesting_name in interesting_names):
                interesting.append(asteroid)
            else:
                regular.append(asteroid)
        
        # Combine interesting ones with a random selection
        result = interesting[:count//2] + regular[:count-len(interesting)]
        return result[:count]
    ...
    def convert_to_rebound_params(self, asteroid_data: Dict):
        ...
        a = asteroid_data.get('a', 1.0)  # Semi-major axis in AU
        e = asteroid_data.get('e', 0.0)  # Eccentricity
        inc = np.radians(asteroid_data.get('i', 0.0))  # Inclination
        Omega = np.radians(asteroid_data.get('Node', 0.0))  # Longitude of ascending node
        omega = np.radians(asteroid_data.get('Peri', 0.0))  # Argument of periapsis
        M = np.radians(asteroid_data.get('M', 0.0))  # Mean anomaly
        ...
        H = asteroid_data.get('H', 20.0)
        estimated_diameter_km = 1329.0 / math.sqrt(0.25) * 10**(-0.2 * H)  # Rough estimate
        estimated_mass_kg = (4/3) * math.pi * (estimated_diameter_km * 500)**3 * 2000  # Assume 2 g/cm³ density
        m_earth_kg = 5.972e24
        ...
        visual_size = max(0.001, 0.01 * (25 - H) / 10)  # Smaller asteroids are tiny
