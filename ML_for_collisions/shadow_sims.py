...
def perfect_merge(sim_pointer, collided_particles_index):
    ...
    merged_radius = (ps[i].r**3 + ps[j].r**3)**(1/3) # calculate merged radius assuming a uniform density

    ps[i] = merged_planet   # update p1's state vector
...
if __name__ == "__main__":
    ...
    original_sim = initialize_sim()
    shadow_run = 0
    while shadow_run < 1000:
        try:
            filename = '/scratch/gpfs/cl5968/ML_data/shadow_runs/sa' + str(run_num) + '_' + str(shadow_run) + '.bin'
            if shadow_run == 0:
                sim = original_sim.copy()
                sim.collision_resolve = perfect_merge
            else:
                sim = original_sim.copy()
                sim.collision_resolve = perfect_merge
                ...
                ps = sim.particles
                ps[1].x = ps[1].x*np.random.normal(1.0, 1e-12)
                ps[1].y = ps[1].y*np.random.normal(1.0, 1e-12)
                ps[1].z = ps[1].z*np.random.normal(1.0, 1e-12)
                ps[1].vx = ps[1].vx*np.random.normal(1.0, 1e-12)
                ps[1].vy = ps[1].vy*np.random.normal(1.0, 1e-12)
                ps[1].vz = ps[1].vz*np.random.normal(1.0, 1e-12)
                ps[2].x = ps[2].x*np.random.normal(1.0, 1e-12)
                ps[2].y = ps[2].y*np.random.normal(1.0, 1e-12)
                ps[2].z = ps[2].z*np.random.normal(1.0, 1e-12)
                ps[2].vx = ps[2].vx*np.random.normal(1.0, 1e-12)
                ps[2].vy = ps[2].vy*np.random.normal(1.0, 1e-12)
                ps[2].vz = ps[2].vz*np.random.normal(1.0, 1e-12)
                ps[3].x = ps[3].x*np.random.normal(1.0, 1e-12)
                ps[3].y = ps[3].y*np.random.normal(1.0, 1e-12)
                ps[3].z = ps[3].z*np.random.normal(1.0, 1e-12)
                ps[3].vx = ps[3].vx*np.random.normal(1.0, 1e-12)
                ps[3].vy = ps[3].vy*np.random.normal(1.0, 1e-12)
                ps[3].vz = ps[3].vz*np.random.normal(1.0, 1e-12)
            ...
            # delete things that destabilize before 10^4 or not at all retroactively
            if num_saves < 101:
                os.remove(filename)
                
                # re-generate initial conditions if this is the original sim
                if shadow_run == 0:
                    original_sim = initialize_sim()
